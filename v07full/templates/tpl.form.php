...
<tr>
	...
	switch($widget) {
		...
		case WIDGET_LANGS: $langs = getLangs(); ?>
		<select class="imgselect" name="<?php echo $prefix;?>[<?php echo $key;?>]" id="<?php echo $key;?>">
			<?php
			foreach($langs as $lang) {
				echo '<option value='. $lang['abbr'] . ' title="' .  BASE_URL . 'front/img/langs/' . $lang['abbr']  .'.png">' . $lang['name'] . '</option>';
			}
			?>
		</select>
	<?php break;
	...
	case WIDGET_FLAG: $apiurl = 'http://localhost/langselect/'; ?>
		<select class="imgselect" name="<?php echo $prefix;?>[<?php echo $key;?>]" id="<?php echo $key;?>">
			<?php
			$countries = json_decode(file_get_contents( $apiurl . 'api.php'));
			foreach($countries as $flag => $country) {
				echo '<option value='. $country . ' title="' .  $apiurl . 'flags/' . $flag .'"' . ($country == $value ? " selected='selected'":'') .'>' . preparelang($country) . '</option>';
			}
			?>
	 </select>
	 	<?php break;
		...
		case WIDGET_DATE:
			...
			$nums = $matches[0]; ?>
			<input name="day" class="day" value="<?php echo @$nums[2];?>"> .
			<input name="month" class="month" value="<?php echo @$nums[1];?>"> .
			<input name="year" class="year" value="<?php echo @$nums[0];?>">
			<?php /*?>
